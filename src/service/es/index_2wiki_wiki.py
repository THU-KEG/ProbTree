...
def main(args):
    ...
    for item in tqdm(chain(train, dev, test)):
        for title, sentences in item['context']:
            para = " ".join(sentences)
            data[para] = title
    data = [{"id": i, "text": text, "title": title} for i, (text, title) in enumerate(data.items())]
    json.dump(data, open('2wiki_wikipedia.json', 'w'), indent = 2)
    print(len(data))
    ...
    wikipedia_data = json.load(open('2wiki_wikipedia.json'))
