...
from search.wikipedia import get_wikipedia_text


READ_CACHE = True
CACHE_DIR = './serpapi_cache'
...
def get_question_wiki_snippet(question, cache=None):
    ...
    try:
        ...
        if "error" in cached_query_results["full_results"] and "Your searches for the month are exhausted" in cached_query_results["full_results"]["error"]:
            raise IOError()
